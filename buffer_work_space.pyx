...
"""Author: Robin Robin"""
"""Email: robinsquare42@gmail.com"""
"""Version: 2.0.0"""
"""Coda available at: https://github.com/RobinNil/file_read_backwards"""
...
cdef class BufferWorkSpace:
    """It is a helper module for FileReadBackwards."""
    cdef int chunk_size
    cdef int read_position
    cdef bytes read_buffer
    cdef int n
    cdef int seek_position
    cdef int read_size
    cdef int i
    cdef int l
    cdef fp

    def __init__(self, fp, int chunk_size):
    ...
    def add_to_buffer(self, bytes content, int read_position):
...
def _get_next_chunk(fp, int previously_read_position, int chunk_size):
...
def _get_what_to_read_next(fp, int previously_read_position, int chunk_size):
